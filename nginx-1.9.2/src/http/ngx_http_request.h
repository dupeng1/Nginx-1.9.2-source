...
typedef struct { //见ngx_http_headers_in
    //匹配头部行name:value中的name，见ngx_http_process_request_headers
    ngx_str_t                         name; 
    ...
    //各个头部对应的handler见ngx_http_headers_in
...
/*
 ...
 5. Authorization：当客户端接收到来自WEB服务器的 WWW-Authenticate 响应时，该头部来回应自己的身份验证信息给WEB服务器，网关用来验证登录态。
...
typedef struct { 
    ...
    */
    ...
    每一个元素都是ngx_table_elt_t成员*/ 
    //从ngx_http_headers_in获取变量后存储到该链表中，链表中的成员就是下面的各个ngx_table_elt_t成员
    //HTTP2的相关头部赋值见ngx_http_v2_state_process_header
    //在ngx_http_process_request_line初始化list空间  ngx_http_process_request_headers中存储解析到的请求行value和key
    ngx_list_t                        headers; 
    ...
    //http1.0以上必须带上host头部行，见ngx_http_process_request_header
    ...
    */
    ...
    但是有所不同的是Last-Modified和HTTP-If-Modified-Since只判断资源的最后修改时间，而ETags和If-None-Match可以是资源任何的任何属性。
    ETags和If-None-Match的工作原理是在HTTPResponse中添加ETags信息。当客户端再次请求该资源时，将在HTTPRequest中加入If-None-Match信息（ETags的值）。
    ...
    */
    ...
    //赋值ngx_http_process_header_line
    ...
    */
    ...
    //server和host指向内容一样，都是头部中携带的host头部
    //server名称   ngx_http_process_host
    ngx_str_t                         server;

    /* 
    在丢弃包体的时候(见ngx_http_read_discarded_request_body)，headers_in成员里的content_length_n，最初它等于content-length头部，
    而每丢弃一部分包体，就会在content_length_n变量中减去相应的大小。因此，content_length_n表示还需要丢弃的包体长度，这里首先检查请求的content_length_n成员，
    如果它已经等于0，则表示已经接收到完整的包体 
    */
    //解析完头部行后通过ngx_http_process_request_header来开辟空间从而来存储请求体中的内容，表示请求包体的大小，如果为-1表示请求中不带包体
    //根据ngx_table_elt_t *content_length计算出的HTTP包体大小  默认赋值-1
    off_t                             content_length_n; 

    //头部行Keep-Alive:所带参数 Connection=keep-alive时才有效  赋值ngx_http_process_request_header
    time_t                            keep_alive_n; 

    /*HTTP连接类型，它的取值范围是0、NGX_http_CONNECTION_CLOSE或者NGX_HTTP_CONNECTION_KEEP_ALIVE*/
    //NGX_HTTP_CONNECTION_KEEP_ALIVE等，表示长连接 短连接
...
*/
typedef struct { 
    //包含在ngx_http_request_s结构headers_out中,ngx_http_send_header中把HTTP头部发出
    /*常用的HTTP头部信息可以通过r->headers_in获取，不常用的HTTP头部则需要遍历r->headers_in.headers来遍历获取*/ 
    //使用查找方法可以参考headers_in
    //如果连接了后端(例如fastcgi到PHP服务器),里面存储的是后端服务器返回的一行一行的头部行信息,
    //赋值在ngx_http_upstream_process_headers->ngx_http_upstream_copy_header_line
    //待发送的HTTP头部链表，与headers_in中的headers成员类似  
    ngx_list_t                        headers;

    /*响应中的状态值，如200表示成功。NGX_HTTP_OK */ //真正发送给客户端的头部行组包生效在ngx_http_status_lines
    ngx_uint_t                        status;
    ...
    /*
     ...
     4. Age：当代理服务器用自己缓存的实体去响应请求时，用该头部表明该实体从产生到现在经过多长时间了（年龄）。
    ...
    */
    ...
    //存储redirect时候，转换后的字符串值，见ngx_http_script_regex_end_code
    ngx_table_elt_t                  *location; 
    ngx_table_elt_t                  *refresh;
    ngx_table_elt_t                  *last_modified;
    ngx_table_elt_t                  *content_range;
    //Accept-Ranges: bytes  应该是content_length的单位
...
ngx_http_post_subrequest_pt回调方法中的rc参数是子请求在结束时的状态，它的取值则是执行ngx_http_finalize_request销毁请求时传递的rc参数
*/
typedef struct { //一个子请求结束时对应的回调
...
/*
    多个ngx_http_postponed_request_t之间使用next指针连接成一个单向链表。ngx_http_postponed_request_t中
...
struct ngx_http_postponed_request_s { //参考ngx_http_postpone_filter方法 
    ...
    ngx_http_request_t               *request; //ngx_http_subrequest创建subrequest的时候，request保存子请求
    ngx_chain_t                      *out;//out成员是ngx_chain_t结构，它指向的是来自上游的、将要转发给下游的响应包体。该请求自己产生的数据
    ngx_http_postponed_request_t     *next; //同一个父请求下创建多个子请求的时候，通过该next连接起来
};
...
NGX_HTTP_LOG_PHASE)是允许任何一个HTTP模块实现自己的ngx_http_handler_pt处理方法，并将其加入到这7个阶段中去的。
在调用HTTP模块的postconfiguration方法向这7个阶段中添加处理方法前，需要先将phases数组中这7个阶段里的handlers动态数组初始化
...
*/ 
//NGX_HTTP_CONTENT_PHASE阶段的处理回调函数ngx_http_handler_pt比较特殊，见ngx_http_core_content_phase 
typedef ngx_int_t (*ngx_http_handler_pt)(ngx_http_request_t *r); //参数r包含了那些Nginx已经解析完的用户请求信息
...
//当接收到客户端请求数据后，调用ngx_http_create_request中创建并赋值
struct ngx_http_request_s { 
   ...
   //指向存放所有HTTP模块的上下文结构体的指针数组,实际上发送给客户端的应答完成后，会把ctx全部置0
    void                            **ctx; 
...
//通过ngx_http_upstream_cache_get获取
    ...
    */ 
    ...
    //接收HTTP请求中包体的数据结构，为NULL表示还没有分配空间
    ...
    */ 
    ...
    //对应客户端请求中请求行的请求方法GET、POS等，取值见NGX_HTTP_GET,也可以用下面的method_name进行字符串比较
    ngx_uint_t                        method; 
    /*
    ...
    */
    ...
    /* HTTP2的method，赋值见ngx_http_v2_parse_method，在组新的HTTP2头部行后，赋值见ngx_http_v2_construct_request_line */ 
    ...
    */
    ...
    /*
    ngx_http_core_main_conf_t->variables数组成员的结构式ngx_http_variable_s， 
    ngx_http_request_s->variables数组成员结构是ngx_variable_value_t,这两个结构的关系很密切，一个所谓变量，一个所谓变量值
    ...
    */ 
    ...
    //所有头部行内容之和，可以参考ngx_http_header_filter
    size_t                            header_size; 

    //HTTP请求的全部长度，包括HTTP包体
    off_t                             request_length; 

    //错误码，取值为NGX_HTTP_BAD_REQUEST等
    ...
    */
    ...
    /* 如果AIO上下文中还在处理这个请求，blocked必然是大于0的，这时ngx_http_close_request方法不能结束请求 
        ngx_http_copy_aio_handler会自增，当内核把数据发送出去后会在ngx_http_copy_aio_event_handler自减
     */
    //阻塞标志位，目前仅由aio使用为0，表示没有HTTP模块还需要处理请求
    unsigned                          blocked:8; 
    ...
    //头部行解析不正确，见ngx_http_parse_header_line
    ...
    //标志位，为1时表示URL发生过rewrite重写  只要不是rewrite xxx bbb sss;aaa不是break结束都会置1
    ...
    //client_body_in_single_buffer on | off;设置
     ...
     */
    //"client_body_in_file_only on |clean"设置 和request_body_no_buffering是互斥的
    unsigned                          request_body_in_file_only:1; 

    //"client_body_in_file_only on"设置
    unsigned                          request_body_in_persistent_file:1; 

    //"client_body_in_file_only clean"设置
    ...
    //是否缓存HTTP包体，如果不缓存包体，和request_body_in_file_only是互斥的，见ngx_http_read_client_request_body
    ...
    */
    ...
    //解析到的一行http头部行中的一行的name开始处 
    //赋值见ngx_http_parse_header_line
    u_char                           *header_name_start; 

    //header_name_start指向Accept-Language:zh-cn中的:处
    //解析到的一行http头部行中的一行的name的尾部 //赋值见ngx_http_parse_header_line
    u_char                           *header_name_end; 

    //header_start指向Accept-Language:zh-cn中的z字符处
    u_char                           *header_start;

    //header_end指向Accept-Language:zh-cn中的末尾换行处
    u_char                           *header_end;
    ...
    //同的是，method_end指向方法名的最后一个字符，而uri_end指向URI结束后的下一个地址，也就是最后一个字符的下一个字符地址（HTTP框架的行为），
    //这是大部分u_char*类型指针对“xxx_start”和“xxx_end”变量的用法。
    //HTTP2的赋值见ngx_http_v2_parse_path
    u_char                           *uri_start;
